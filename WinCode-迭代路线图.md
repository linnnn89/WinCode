...
| 版本 | 范围与状态 | 验证边界 |
| --- | --- | --- |
| 0.9.1 / R1 | 已实现有界工作区摘要与按需目录工具，准备 PR | 非交互回归 169 pass、1 skip；最终入口筛选修正后专项 9/9。新编译 Gateway 独立 MCP 会话打开真实 TavernDesk，保留 solution、应用项目入口；不是 Codex 当前连接升级或 GUI 验收 |
| R2–R6 | 按序推进；R2 构建身份模块在独立文件准备 | 未完成版本不得据此宣称已验证 |
