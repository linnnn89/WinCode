...
try {
  ...
  await scenario('three independent processes: parallel cold load and exact references', async () => {
    await sample('cold');
    const targets = await Promise.all(clients.map(c => search(c)));
  ...
  await scenario('128 queued semantic searches in one process with independent sibling traffic', async () => {
    const work = Promise.all(Array.from({ length: 128 }, () => search(a)));
    const health = await ok(a, 'wincode_hello_world');
    await Promise.all([work, search(b), search(a2)]);
    await sample('after-128');
    return { sampledInFlight: health.health.inFlightRequests };
  });
  await scenario('64-request burst with 16 cancellations preserves sibling work and warm snapshots', async () => {
    ...
    for (const v of values.filter(v => v.index % 4 !== 0)) {
      assert.ok(v.response && !v.response.error, JSON.stringify(v));
      assert.equal(v.response.data.symbols[0].signature, 'A.Api.Save(int)');
    }
    for (const c of clients) assert.equal((await search(c)).location.snapshotId, c.target.location.snapshotId);
    return { requestedCancellations: 16, rejected: values.filter(v => v.rejected).length, responses: values.filter(v => v.response).length };
  });
  await scenario('shared-instance interleaving: A opens A, B opens B, A queries by name', async () => {
    ...
    assert.equal(after.symbols[0].signature, 'B.Api.Save(int)');
    const stale = await call(a, 'wincode_find_references', { symbolName: 'Save', symbolLocation: before.location });
    assert.equal(stale.error, true); assert.equal(stale.data.errorCode, 'SNAPSHOT_STALE');
    ...
    report.findings.push({ id: 'shared-workspace-context', observed: true, expectedAgentProject: 'A', actualSignature: after.symbols[0].signature,
  ...
  await scenario('native Tray accepts eight registrations, rejects the ninth, and recovers a freed slot', async () => {
    ...
    async function peer(show = false) {
      ...
      const ack = await new Promise((resolve, reject) => {
        ...
        socket.on('connect', () => socket.write(JSON.stringify(show ? { v: 1, type: 'show' } : {
          v: 1, type: 'register', instanceId: id, pid: process.pid, version: '0.14.0', buildId: 'capacity-fixture',
    ...
    try {
      const peers = [];
      for (let i = 0; i < 8; i++) { const p = await peer(); assert.equal(p.ack.type, 'register-accepted'); peers.push(p); }
