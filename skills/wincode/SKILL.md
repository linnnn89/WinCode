...
源码契约：0.13.0（含 E4 开发中错误 JSON 迁移，状态见诊断手册）；手册修订：2026-09-09（本地待发布）。外部 Serena 入口与旧 source 已退役，不能将此版本号当作当前连接已升级。安装内容可用 `node scripts/sync-skill.mjs <安装目录绝对路径>` 核对；仅维护时执行。以当前连接实际 Schema 为准。
