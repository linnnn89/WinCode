...
/** Mask comments and string/character literals without changing offsets or line numbers. */
function maskNonCode(source: string, checkpoint: () => void, maxDepth: number): string | null {
...
export async function mapUiCodeCandidates(
  ...
  for (const file of candidateCodeFiles) {
    ...
    try {
      ...
      const code = maskNonCode(source, checkpoint, result.limits.maxInterpolationDepth);
